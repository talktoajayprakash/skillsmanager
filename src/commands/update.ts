  ...
  if (!collection) {
    ...
    if (!entry) {
      if (entries.length > 1) {
        ...
        console.log(chalk.dim(`  Use: sm update ${skillPath} --collection <name>`));
      } else {
        console.log(chalk.red(`Skill "${skillName}" is not tracked in the skills index.`));
        console.log(chalk.dim(`  Run: sm fetch ${skillName} --agent <agent> first.`));
      }
      return;
    }
    ...
    if (!collection) {
      console.log(chalk.red(`Collection not found. Run: sm refresh`));
      return;
    }
  }
